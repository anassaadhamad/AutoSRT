...
namespace AutoSRTDesktop
{
        ...
        public static async Task<string> ExtractAudioAsync(string ffmpegExe, string inputPath, CancellationToken ct, Action<string> logCallback)
        {
            ...
            try
            {
                ...
                if (process.ExitCode != 0)
                {
                    ...
                    {
                        throw new Exception("الملف لا يحتوي على أي مسار صوتي (Audio track) لتفريغه.");
                    }
                    throw new Exception("فشل استخراج الصوت عبر FFmpeg: كود الخطأ " + process.ExitCode);
                }

                if (!File.Exists(outputMp3) || new FileInfo(outputMp3).Length == 0)
                {
                    throw new Exception("لم يتم إنتاج ملف صوتي صالح.");
                }
    ...
    public static class GroqClient
    {
            ...
            Action<string> statusCallback)
        {
            ...
            {
                throw new Exception("مفتاح Groq API غير محدد. يرجى إدخال المفتاح أولاً من شريط الإعدادات بالأعلى.");
            }
            ...
            {
                throw new Exception(string.Format("حجم الملف الصوتي ({0:0.1} MB) يتجاوز الحد الأقصى لواجهة Groq (25 MB). يرجى تقليل مدة الملف أو ضغطه.", fi.Length / (1024.0 * 1024.0)));
            }

            statusCallback("جاري الاتصال بـ Groq Whisper...");
            ...
            using (var form = new MultipartFormDataContent())
            {
                ...
                // Request direct SRT output from Whisper
                ...
                using (var request = new HttpRequestMessage(HttpMethod.Post, "https://api.groq.com/openai/v1/audio/transcriptions"))
                {
                    ...
                    statusCallback("جاري التفريغ وتحويل الكلام إلى نص عبر الذكاء الاصطناعي...");
                    ...
                    using (var response = await client.SendAsync(request, HttpCompletionOption.ResponseContentRead, ct))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            {
                                throw new Exception("لم يتم استلام أي نص ترجمة من الذكاء الاصطناعي (الملف قد يكون صامتًا).");
                            }
                            return srtContent;
                        }
                        ...
                        {
                            throw new Exception("مفتاح Groq API غير صالح (Unauthorized). تأكد من صحة المفتاح.");
                        }
                        else if ((int)response.StatusCode == 429)
                        {
                            throw new Exception("تم تجاوز الحد المسموح للطلبات (Rate Limit) في Groq. انتظر بضع ثوانٍ وأعد المحاولة.");
                        }
                        else
                        {
                            throw new Exception("خطأ من Groq API (" + (int)response.StatusCode + "): " + errBody);
                        }
                    }
                }
            }
    ...
    public class MainForm : Form
    {
        ...
        // UI Colors (Tailwind Dark Theme: Slate & Indigo)
        ...
        {
            this.Text = "AutoSRT Desktop - تفريغ وترجمة الفيديوهات بالذكاء الاصطناعي (Local Edition)";
            ...
            // Try loading app icon if exists
            try
            {
            ...
            Panel pnlSettings = CreateSettingsPanel();
            Panel pnlActions = CreateActionsBar();
        ...
        private Panel CreateHeaderPanel()
        {
            ...
            {
                Text = "⚡ AutoSRT Desktop (نسخة سطح المكتب المستقلة)",
            ...
            {
                Text = "تفريغ وترجمة الفيديوهات والملفات الصوتية إلى ملفات SRT محلياً 100% بدون أي خادم وسيط وبأعلى سرعة",
            ...
            LinkLabel lnkGetFreeKey = new LinkLabel
            {
                Text = "🔑 احصل على مفتاح Groq مجاني (بدون فيزا)",
        ...
        private Panel CreateSettingsPanel()
        {
            ...
            // Row 1: API Key & Model & Language
            Label lblKey = new Label { Text = "مفتاح Groq API:", AutoSize = true, Location = new Point(15, 16), ForeColor = colorTextMuted };
            ...
            Label lblModel = new Label { Text = "النموذج:", AutoSize = true, Location = new Point(445, 16), ForeColor = colorTextMuted };
            ...
            cmbModel.Items.Add("whisper-large-v3-turbo (فائق السرعة)");
            cmbModel.Items.Add("whisper-large-v3 (أعلى دقة)");
            cmbModel.SelectedIndex = settings.Model.Contains("turbo") ? 0 : 1;
            ...
            Label lblLang = new Label { Text = "اللغة:", AutoSize = true, Location = new Point(705, 16), ForeColor = colorTextMuted };
            ...
            cmbLang.Items.Add("تلقائي (Auto)");
            cmbLang.Items.Add("العربية (ar)");
            cmbLang.Items.Add("الإنجليزية (en)");
            cmbLang.Items.Add("الفرنسية (fr)");
            cmbLang.Items.Add("الألمانية (de)");
            cmbLang.Items.Add("الإسبانية (es)");
            cmbLang.Items.Add("التركية (tr)");
            ...
            {
                Text = "حفظ ملف SRT بجانب ملف الفيديو الأصلي مباشرة",
            ...
            btnBrowseOutput.Click += delegate
            {
                ...
                {
                    fbd.Description = "اختر مجلد حفظ ملفات الترجمة SRT";
        ...
        private void SetSelectedLanguage(string code)
        {
            switch (code)
            {
                case "ar": cmbLang.SelectedIndex = 1; break;
                case "en": cmbLang.SelectedIndex = 2; break;
                case "fr": cmbLang.SelectedIndex = 3; break;
                case "de": cmbLang.SelectedIndex = 4; break;
                case "es": cmbLang.SelectedIndex = 5; break;
                case "tr": cmbLang.SelectedIndex = 6; break;
                default: cmbLang.SelectedIndex = 0; break;
            }
        }
        ...
        private void RefreshFfmpegStatus()
        {
            ...
            if (ready)
            {
                lblFfmpegStatus.Text = "🟢 FFmpeg: متوفر ومفعل محلياً";
                lblFfmpegStatus.ForeColor = colorSuccess;
                btnFfmpegAction.Visible = false;
            }
            else
            {
                lblFfmpegStatus.Text = "🟡 FFmpeg: غير متوفر (مطلوب للفيديوهات)";
                lblFfmpegStatus.ForeColor = Color.FromArgb(251, 191, 36);
                btnFfmpegAction.Text = "تنزيل FFmpeg تلقائياً";
        ...
        private async void BtnFfmpegAction_Click(object sender, EventArgs e)
        {
            ...
            btnFfmpegAction.Text = "جاري التنزيل...";
            lblFfmpegStatus.Text = "⏳ جاري إعداد وتنزيل FFmpeg في الخلفية...";

            try
            {
                await Task.Run(delegate
                {
                    // Download static ffmpeg build from reliable GitHub mirror
                    ...
                    // Extract ffmpeg.exe to AppData/Local/AutoSRT/bin/ffmpeg.exe
                ...
                MessageBox.Show("تم تنزيل وتثبيت FFmpeg محلياً بنجاح!", "AutoSRT", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
                MessageBox.Show("تعذر تنزيل FFmpeg تلقائياً (" + ex.Message + "). يمكنك نسخ ffmpeg.exe إلى نفس مجلد البرنامج أو إضافته إلى PATH.", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            finally
            {
                RefreshFfmpegStatus();
        ...
        private Panel CreateActionsBar()
        {
            ...
            Label lblDragTip = new Label
            {
                Text = "💡 يمكنك سحب وإفلات الملفات أو المجلدات مباشرة داخل هذا الجدول",
                ForeColor = colorTextMuted,
                AutoSize = true,
        ...
        private void BtnAddFiles_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog ofd = new OpenFileDialog())
            {
                ...
                ofd.Filter = "جميع ملفات الميديا|*.mp4;*.mkv;*.mov;*.avi;*.webm;*.flv;*.wmv;*.mp3;*.wav;*.m4a;*.aac;*.flac;*.ogg;*.opus|فيديو|*.mp4;*.mkv;*.mov;*.avi;*.webm;*.flv;*.wmv|صوت|*.mp3;*.wav;*.m4a;*.aac;*.flac;*.ogg;*.opus|كل الملفات|*.*";
        ...
        private void BtnAddFolder_Click(object sender, EventArgs e)
        {
            using (FolderBrowserDialog fbd = new FolderBrowserDialog())
            {
                fbd.Description = "اختر المجلد الذي يحتوي على ملفات الفيديو أو الصوت";
                if (fbd.ShowDialog() == DialogResult.OK)
                {
        ...
        private DataGridView CreateDataGridView()
        {
            ...
            var colName = new DataGridViewTextBoxColumn { HeaderText = "اسم الملف", Width = 260, ReadOnly = true };
            colName.AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
            dgv.Columns.Add(colName);
            dgv.Columns.Add(new DataGridViewTextBoxColumn { HeaderText = "النوع / الحجم", Width = 110, ReadOnly = true });
            dgv.Columns.Add(new DataGridViewTextBoxColumn { HeaderText = "الحالة ومراحل المعالجة", Width = 300, ReadOnly = true });
        ...
        private void Dgv_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (e.ColumnIndex == 5)
            {
                ...
                {
                    MessageBox.Show("لم يتم إنتاج ملف الترجمة SRT بعد لهذا العنصر.", "معاينة الترجمة", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            // Col 6: Delete item
            else if (e.ColumnIndex == 6)
            {
                ...
                {
                    MessageBox.Show("لا يمكن حذف عنصر قيد المعالجة حالياً. قم بإلغاء المعالجة أولاً.", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
        ...
        private void ShowSrtViewer(string fileName, string srtPath)
        {
            ...
            {
                Text = "معاينة ملف الترجمة: " + fileName,
                Size = new Size(700, 550),
                StartPosition = FormStartPosition.CenterParent,
                BackColor = colorBg,
                ForeColor = colorText,
                Font = new Font("Segoe UI", 9.5f),
                RightToLeft = RightToLeft.No // Subtitle timestamps are LTR
            };
        ...
        private void RefreshGrid()
        {
            ...
            {
                var item = queue[i];
                string typeLabel = (item.IsAudioOnly ? "🎵 صوت" : "🎬 فيديو") + " (" + item.FormattedSize + ")";
                grid.Rows.Add(
                    (i + 1).ToString(),
                    item.FileName,
                    typeLabel,
        ...
        private void UpdateSummaryLabel()
        {
            ...
            lblStatus.Text = string.Format("إجمالي الملفات: {0} | المكتمل: {1} | بالانتظار: {2}", queue.Count, readyCount, queue.Count - readyCount);
        }
        #endregion
        ...
        private Panel CreateFooterPanel()
        {
            ...
            lblStatus = new Label
            {
                Text = "جاهز. أضف الملفات واضغط على 'بدء التفريغ والترجمة'.",
                ForeColor = colorTextMuted,
                AutoSize = true,
        ...
        private async void BtnStart_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(apiKey))
            {
                MessageBox.Show("يرجى إدخال مفتاح Groq API الخاص بك في الحقل بالأعلى للبدء.\nيمكنك الحصول عليه مجاناً فوراً عبر الرابط بأعلى الشاشة.", "مفتاح API مطلوب", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtApiKey.Focus();
                return;
            }

            // Find items that need processing (Pending or Failed)
            ...
            {
                MessageBox.Show("لا توجد ملفات بالانتظار للمعالجة. قم بإضافة ملفات فيديو أو صوت أولاً.", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            // Check if any item needs FFmpeg (videos or large files)
            ...
            if (needsFfmpeg && string.IsNullOrEmpty(ffmpegPath))
            {
                var dr = MessageBox.Show(
                    "تتضمن القائمة ملفات فيديو تحتاج إلى استخراج الصوت عبر FFmpeg، ولكن FFmpeg غير مثبت على جهازك.\n\nهل ترغب في تنزيله محلياً تلقائياً الآن؟",
                    "FFmpeg مطلوب",
            ...
            try
            {
                for (int i = 0; i < toProcess.Count; i++)
                {
                    ...
                    lblStatus.Text = string.Format("جاري معالجة الملف ({0} من {1}): {2}", i + 1, toProcess.Count, item.FileName);
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            item.StatusMessage = "🎵 جاري استخراج وضغط الصوت (FFmpeg)...";
                        ...
                        item.StatusMessage = "⚡ جاري التفريغ وتحويل الكلام إلى نص (Groq AI)...";
                        UpdateGridRowStatus(rowIndex, item.StatusMessage, Color.FromArgb(129, 140, 248));
                        ...
                        item.StatusMessage = "✅ تم بنجاح - تم إنشاء ملف SRT";
                    ...
                    catch (OperationCanceledException)
                    {
                        ...
                        item.StatusMessage = "⏹️ تم الإلغاء";
                    ...
                    catch (Exception ex)
                    {
                        ...
                        item.StatusMessage = "❌ خطأ: " + ex.Message;
                        UpdateGridRowStatus(rowIndex, item.StatusMessage, colorDanger);
                    }
                    finally
                    {
                        // Clean up temporary extracted audio file if created
                ...
                prgActive.Value = 100;
                lblStatus.Text = string.Format("اكتملت العملية! تم تفريغ {0} ملف(ات) بنجاح.", completedInThisRun);
            }
            finally
            {
                isProcessing = false;
        ...
        private void BtnCancel_Click(object sender, EventArgs e)
        {
            ...
            {
                btnCancel.Enabled = false;
                lblStatus.Text = "جاري إيقاف وإلغاء العمليات الحالية...";
                cts.Cancel();
            }
        }
